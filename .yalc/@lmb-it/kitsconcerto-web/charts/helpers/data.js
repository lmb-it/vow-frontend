import { labelsWithValue } from './helper.js';
import Colors, { getColorByIndex } from './ColorsObj.js';
...
const generateLineData = (data, isDarkMode) => {
  ...
  !!originalDatasets && originalDatasets.forEach((originalDataset, index) => {
    ...
    datasets.push({
      type: "line",
      label: title,
      borderColor: documentStyle.getPropertyValue(`--${getColorByIndex(index)}-500`),
      borderWidth: 2,
      fill: false,
      tension: 0.4,
      data: dataset
...
const generateBarData = (data, isDarkMode) => {
  ...
  !!originalDatasets && originalDatasets.forEach((originalDataset) => {
    ...
    const color = Colors.pickLevels(originalDatasets.length, isDarkMode, chartType);
    datasets.push({
      label: title,
...
const generatePieData = (data, isDarkMode) => {
  ...
  !!originalDatasets && originalDatasets.forEach((item, _itemIndex, itemsArray) => {
    const colors = Colors.pickLevels(itemsArray.length, isDarkMode, chartType);
    datasets.push({
      label: item.label,
      data: item.data,
  ...
  return {
    labels: originalLabels ? originalLabels : labelsWithValue(originalDatasets),
    datasets
  };
};
...
const generateDoughnutData = (data, isDarkMode) => {
  ...
  datum.forEach((item, _itemIndex, itemsArray) => {
    ...
    datasets.push({
      ...
      label: item.label,
      data: item.data,
      backgroundColor: arr.map((index) => documentStyle.getPropertyValue(`--${getColorByIndex(index)}-500`)),
      hoverBackgroundColor: arr.map((index) => documentStyle.getPropertyValue(`--${getColorByIndex(index)}-400`)),
      hoverOffset: 7.5,
  ...
  return {
    labels: labels ? labels : labelsWithValue(datum),
    datasets
  };
};

export { generateBarData, generateDoughnutData, generateLineData, generatePieData, generatePolarAreaData, generateRadarData };
