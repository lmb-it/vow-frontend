...
const toCssValue = (key, value) => {
  if (typeof value === "string" && resolvers.isThemeToken(value)) {
  ...
  if (Object.values(index_web.sizingProperties).includes(key)) {
  ...
  if (typeof value === "number" && Object.values(index_web.pxProperties).includes(key)) {
    return `${value}px`;
  }
  return value;
};
