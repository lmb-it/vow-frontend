...
function generateCssVariables(theme, colorMode) {
  ...
  for (const [name, ref] of Object.entries(semantics)) {
    const resolved = resolvers.resolveTokenValue(theme, colorMode, ref, "native");
    lines.push(`  --kits-${name}: ${resolved};`);
  }
  const primary = resolvers.resolveTokenValue(theme, colorMode, "primary", "native");
  const text = resolvers.resolveTokenValue(theme, colorMode, "text", "native");
  const textSecondary = resolvers.resolveTokenValue(theme, colorMode, "text-secondary", "native");
  const surfaceGround = resolvers.resolveTokenValue(theme, colorMode, "surface-ground", "native");
  const surfaceCard = resolvers.resolveTokenValue(theme, colorMode, "surface-card", "native");
  const surfaceOverlay = resolvers.resolveTokenValue(theme, colorMode, "surface-overlay", "native");
  const surfaceHover = resolvers.resolveTokenValue(theme, colorMode, "surface-hover", "native");
  const border = resolvers.resolveTokenValue(theme, colorMode, "border", "native");
  const hexToRgb = (hex) => {
    const h = hex.replace("#", "");
...
//# sourceMappingURL=resolvers.web.cjs.map
